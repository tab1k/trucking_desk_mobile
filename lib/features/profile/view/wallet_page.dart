...
import 'package:fura24.kz/features/payments/data/repositories/payments_repository.dart';
...
class _WalletPageState extends ConsumerState<WalletPage> with WidgetsBindingObserver {
  ...
  Future<void> _showTopUpSheet() async {
    ...
    // Clean up controller when sheet closes is tricky with local var, 
    // but usually flutter handles disposal of text controllers if in state.
    // For a modal, we can just let it be garbage collected or dispose if we make a proper widget.
    // simpler: allow it to leak slightly or use a Widget class. 
    // Let's use a separate widget class for the Sheet to be clean and handle state.
    ...
    await showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: _TopUpSheet(onPay: _initiatePayment),
      ),
    );
  }
  ...
  Future<void> _initiatePayment(double amount) async {
    try {
      ...
      final payment = await repo.createPayment(amount);
      ...
      if (payment.redirectUrl != null) {
        ...
        if (await canLaunchUrl(uri)) {
          await launchUrl(uri, mode: LaunchMode.externalApplication);
           ...
           if (mounted) {
             ScaffoldMessenger.of(context).showSnackBar(
               const SnackBar(content: Text('Не удалось открыть ссылку на оплату')),
             );
           }
        }
      } else {
         if (mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
             const SnackBar(content: Text('Ошибка: нет ссылки на оплату')),
           );
         }
      }
      ...
      // Hide loading
      if (mounted) {
        Navigator.of(context, rootNavigator: true).pop(); // Try to pop loading if still there
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    // ... (rest of the build method same mostly)
          ...
          child: Column(
            ...
            children: [
              ...
              _ActionTile(
                ...
                subtitle: 'Банковской картой', // Changed from "Soon"
...
class _TopUpSheet extends StatefulWidget {
  const _TopUpSheet({required this.onPay});
...
class _TopUpSheetState extends State<_TopUpSheet> {
  ...
  @override
  Widget build(BuildContext context) {
    final commission = _amount * _commissionRate;
    final total = _amount + commission;
    final format = NumberFormat.currency(locale: 'ru_RU', symbol: '₸', decimalDigits: 0);
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            inputFormatters: [
              // Optionally add formatters if needed, but simple number is fine for now
            ],
          ),
          SizedBox(height: 12.h),
